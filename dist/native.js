...
async function copyToNativeProject(platform, resourcesDirectory, nativeProject, shouldCopyIcons, shouldCopySplash, logstream, errstream) {
    ...
    if (platform === "ios" /* IOS */) {
        ...
        const resPath = ANDROID_RES_PATH.replace('{FLAVOR}', nativeProject.flavor || 'main');
        if (shouldCopyIcons) {
            count += await copyImages(path_1.default.join(resourcesDirectory, SOURCE_ANDROID_ICON), path_1.default.join(androidProjectDirectory, resPath), ANDROID_ICONS, errstream);
        }
        if (shouldCopySplash) {
            count += await copyImages(path_1.default.join(resourcesDirectory, SOURCE_ANDROID_SPLASH), path_1.default.join(androidProjectDirectory, resPath), ANDROID_SPLASHES, errstream);
        }
    }
    else {
